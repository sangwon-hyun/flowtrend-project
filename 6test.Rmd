...
testthat::test_that("Objective value decreases over EM iterations.",{
  ...
  cowplot::plot_grid(title, main_plot, ncol=1, rel_heights=c(0.1, 1))
})
```
...
timelist = c(10, 20, 30, 40, 60, 80)
  plot_2d(dt$ylist, obj = obj, tt = tt, bin = FALSE) + coord_fixed() + ylim(-6, 3) + xlim(-6, 3)
})
...
plot_2d(ylist = binres$ybin_list,
        countslist = binres$counts_list,
        obj = obj, tt = tt, bin = FALSE) + coord_fixed
```

Also plot the cluster probabilities:

```{r binned-prob-plot, fig.width = 7, fig.height = 7}
plot_prob(obj)
